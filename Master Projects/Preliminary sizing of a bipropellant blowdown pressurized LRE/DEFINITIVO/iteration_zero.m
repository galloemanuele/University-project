...
y=CEA('problem','rocket','frozen','nfz',frozen,'o/f',o_f,'case','RP1_LOX','p,bar',Pc_in,'sup',epsilon,'reactants','fuel','RP-1(L)','C',1,'H',1.9423,'wt%',100,'t(k)',300,'h,cal/mol',-5430,'oxid','O2(L)','t(k)',90.17,'output','transport','mks','short','end','screen');

gamma_in=y.output.froz.gamma(2); % throat
...
gamma_gc_gola_in=y.output.froz.gamma(2); % In throat
Mm_gc_gola_in=y.output.froz.mw(2);       % In throat==chamber
Tf_gola_in=y.output.froz.temperature(2); % In chamber
rho_gola_in=y.output.froz.density(2);    % In chamber
...
f=@(Pe) (((gamma_in+1)/2)^(1/(gamma_in-1))) * ((Pe/Pc_in)^(1/gamma_in)) *...
 ...
 Pe_guess=y.output.froz.pressure(3)*1e5;
...
%% dim CC
L_star = 1.27;  % worst case scenario (?)
eps_c = 10;     % tbd range from 5 to 15
Ac = eps_c*At;
Dc = sqrt(4*Ac/pi);
Vc = L_star*At;
Lc = Vc/Ac;
...
N_fu = floor(A_fu/A_inj_fu)
A_fu_new = A_inj_fu*N_fu;
m_FU_new = A_fu_new*cD_fu*sqrt(2*d_p*rho_FU);

% doublet
% d_ox = 0.0005;
% A_inj_ox = pi*d_ox^2/4;
% A_ox = N_fu * A_inj_ox;
% cD_ox = m_OX/(A_ox*sqrt(2*d_p*rho_OX));
% 
% d_p_f = 0.15 * 2e6;
% m_fu_f = A_fu*sqrt(2*rho_FU*d_p_f);
% m_ox_f = A_ox*sqrt(2*rho_OX*d_p_f);
% m_ox_f/m_fu_f
% m_ox_f+m_fu_f
...
for i=1:length(ratio)
    if(GHIRINGHELLI(i,end)<2.8 && GHIRINGHELLI(i,end)>2.5 )
...
% capra è la riga di brandonisio per cui o/f=2.67 , valor medio tra massimo
% e minimo
%%
clc
capra=brandonisio(37,:)
...
save("RISULTATI_ITERAZIONE1.mat")