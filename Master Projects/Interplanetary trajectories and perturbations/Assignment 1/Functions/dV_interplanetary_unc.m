function [Dv,r_dep,r_flyby,r_arr,Dv1_vect,Dv2_vect,vt1_in,vt2_in,vt1_out,vt2_out,v_inf_minus,v_inf_plus] = dV_interplanetary_unc(t1,t2,t3)
% DESCRIPTION:
%  Function to compute the total cost of the mission without considering
%  the flyby cost and constraints
% --------------------------------------------------------------------------
% PROTOTYPE:
%  [Dv,r_dep,r_flyby,r_arr,Dv1_vect,Dv2_vect,vt1_in,vt2_in,vt1_out,vt2_out,
%  v_inf_minus,v_inf_plus] = dV_interplanetary_unc(t1,t2,t3)
% --------------------------------------------------------------------------
% INPUT:
%  t1[1]          Departure date in MJD2000                                [days]
%  t2[1]          Flyby date  in MJD2000                                   [days]
%  t3[1]          Arrival date  in MJD2000                                 [days]
% --------------------------------------------------------------------------
% OUTPUT:
...
if t2>t1 && t3>t2    
        ...
        [~, ~, ~, ERROR1, vt1_in, vt1_out, ~, ~] = lambertMR(r_dep, r_flyby, (t2-t1) * (24*3600), mu_Sun, 0, 0, 0, 2);
        ...
        [~,~,~,ERROR2, vt2_in, vt2_out,~,~] = lambertMR(r_flyby, r_arr,(t3-t2) * (24*3600), mu_Sun, 0, 0, 0, 2);
